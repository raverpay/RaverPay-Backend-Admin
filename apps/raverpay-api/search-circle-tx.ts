...
async function main() {
  ...
  console.log('Search by circleTransactionId:', byCircleTxId ? 'FOUND' : 'NOT FOUND');
  ...
  recentTxs.forEach((tx, i) => {
    console.log(`${i + 1}. ID: ${tx.circleTransactionId}, refId: ${tx.refId}, state: ${tx.state}, created: ${tx.createdAt}`);
  });
}
