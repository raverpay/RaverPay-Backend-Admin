...
async function runRaceConditionTest() {
  ...
  try {
    ...
    const axiosError = error as AxiosError<any>;
    console.error('   ❌ Failed to get wallet:', axiosError.response?.data?.message);
    return;
  }
  ...
  console.log(`   Total debited:    ₦${(startBalance - finalBalance).toLocaleString()}\n`);
}

// ============== WITHDRAWAL TEST ==============
async function testWithdrawalRaceCondition(accessToken: string, balance: number) {
  console.log('═══════════════════════════════════════════════════════════');
  ...
  if (balance < requiredAmount) {
    console.log(`   ⚠️  Skipping - Need ₦${requiredAmount}, have ₦${balance}\n`);
    return;
  }
  ...
  console.log(`   Firing ${CONFIG.CONCURRENT_REQUESTS} concurrent withdrawals...\n`);
  
  const startTime = Date.now();
  const promises = Array(CONFIG.CONCURRENT_REQUESTS).fill(null).map((_, i) => {
    return api.post(
      ...
      index: i + 1, success: true, data: res.data, ms: Date.now() - startTime,
    })).catch(err => ({
      index: i + 1, success: false, error: err.response?.data?.message || err.message, ms: Date.now() - startTime,
    }));
  });
...
async function testAirtimeRaceCondition(accessToken: string, balance: number) {
  ...
  if (balance < CONFIG.AIRTIME_AMOUNT * 2) {
    console.log(`   ⚠️  Skipping - Need ₦${CONFIG.AIRTIME_AMOUNT * 2}, have ₦${balance}\n`);
    return;
  }
  
  console.log(`   Target: ${CONFIG.VTU_PHONE} (${CONFIG.VTU_NETWORK})`);
  console.log(`   Firing ${CONFIG.CONCURRENT_REQUESTS} concurrent airtime purchases...\n`);
...
function printResults(testName: string, results: any[]) {
  ...
  results.forEach((r: any) => {
    if (r.success) {
      console.log(`   Request ${r.index}: ✅ SUCCESS (${r.ms}ms) - Ref: ${r.data.reference || r.data.transactionId || 'N/A'}`);
    } else {
  ...
  if (successful.length > 1) {
    ...
    console.log(`   ${successful.length} requests succeeded when only 1 should have!\n`);
