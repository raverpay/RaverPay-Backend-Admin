...
import { Card, isWeakPin, PinPad, ScreenHeader, Text } from '@/src/components/ui';
    ...
    changePinMutation(
      ...
      },
      {
        ...
        onError: (error: any) => {
          setError(true);
          const errorMessage =
            error?.response?.data?.message || 'Failed to change PIN. Please try again.';
      ...
      <ScrollView className="flex-1 px-5 pt-8" showsVerticalScrollIndicator={false}>
        ...
        <View className="flex-row items-center justify-center mb-8">
          <View className="flex-row items-center">
            ...
            <View className={`w-12 h-0.5 ${step === 'current' ? 'bg-gray-300' : 'bg-green-500'}`} />
            <View
              className={`w-8 h-8 rounded-full items-center justify-center ${
                step === 'new'
                  ? 'bg-[#5B55F6]'
            ...
            <View className={`w-12 h-0.5 ${step === 'confirm' ? 'bg-[#5B55F6]' : 'bg-gray-300'}`} />
            <View
              className={`w-8 h-8 rounded-full items-center justify-center ${
          ...
          <View className="flex-row items-start">
            ...
            <View className="flex-1 ml-2">
              <Text variant="caption" className="text-blue-800">
                Your transaction PIN is used to authorize payments and withdrawals. Keep it secure
