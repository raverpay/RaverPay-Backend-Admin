...
export default function DevToolsScreen() {
  ...
  const testUnhandledError = () => {
    Alert.alert('⚠️ Warning', 'This will crash the app! Are you sure?', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: () => {
          setTimeout(() => {
            throw new Error('Test unhandled error from dev tools - App will crash!');
          }, 100);
        },
      },
    ]);
  ...
  const testBreadcrumbs = () => {
    ...
    setTimeout(() => {
      ...
      Alert.alert('🍞 Breadcrumbs', '3 breadcrumbs added, error sent to Sentry');
    }, 1000);
  };
  ...
  const testNetworkError = async () => {
    try {
      ...
      Alert.alert('🌐 Network Error', 'Network error captured and sent to Sentry');
    }
  };
  ...
  const testPaymentError = () => {
    ...
    setErrorCount((prev) => prev + 1);
    Alert.alert('💳 Payment Error', 'Payment error captured and sent to Sentry');
  };
  ...
  const testVtuError = () => {
    const error = new Error('VTU provider error: Service temporarily unavailable');
    captureException(error, {
      tags: {
        test: 'true',
  ...
  const triggerMultipleErrors = () => {
    Alert.alert('🔥 Trigger Alert', 'This will send 5 errors rapidly to test alert rules', [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Send 5 Errors',
        style: 'destructive',
  ...
  return (
    <View className="flex-1 bg-white dark:bg-background">
      ...
      <ScreenHeader title="Sentry Dev Tools" onBack={() => router.back()} />

      <ScrollView className="flex-1 p-6">
        <View className="mb-6">
          <Text className="text-2xl font-bold mb-2 dark:text-white">🛠️ Sentry Test Tools</Text>
          <Text className="text-gray-600 dark:text-gray-400">
            Test Sentry integration in development mode
          </Text>
          <Text className="text-primary font-semibold mt-2">Errors sent: {errorCount}</Text>
        </View>
        ...
        <View className="gap-4">
          ...
          >
            ...
            <Text className="text-white/80 text-center text-xs mt-1">Simulates API failure</Text>
          </TouchableOpacity>
          ...
          >
            ...
            <Text className="text-white/80 text-center text-xs mt-1">VTU provider unavailable</Text>
          </TouchableOpacity>
        ...
        <View className="mt-8 p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-xl border border-yellow-200 dark:border-yellow-800">
          ...
          <Text className="text-yellow-700 dark:text-yellow-300 text-xs">
            These test buttons are only visible in development mode. After testing, check your
            Sentry dashboard for captured errors.
          </Text>
        </View>
        ...
        <View className="mt-4 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-200 dark:border-blue-800">
          ...
          <Text className="text-blue-700 dark:text-blue-300 text-xs">
            • Sentry is now enabled in dev mode{'\n'}• Check breadcrumbs in Sentry dashboard under
            each error{'\n'}• Session replays capture screen content (privacy-safe){'\n'}• Use
            &quot;Test Handled Error&quot; to avoid app crashes{'\n'}• Alert rules test will send 5
            errors in 2.5 seconds
          </Text>
        </View>
