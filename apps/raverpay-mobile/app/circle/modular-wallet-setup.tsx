...
export default function ModularWalletSetup() {
  ...
  const [currentStep, setCurrentStep] = useState<'register' | 'wallet' | 'complete'>('register');
    ...
    try {
      ...
      switch (type) {
        ...
        case 'error':
          ...
          Alert.alert('Error', data.message || 'An error occurred', [{ text: 'OK' }]);
          break;
      ...
      <View className="flex-1 bg-gray-50 dark:bg-gray-900">
        ...
        <View className="px-6 py-4 bg-white dark:bg-gray-800">
          <View className="flex-row items-center justify-between">
            ...
            <View className="flex-1 items-center">
              <View
                className={`w-10 h-10 rounded-full items-center justify-center ${
                  currentStep === 'complete' ? 'bg-green-500' : 'bg-gray-300 dark:bg-gray-600'
                }`}
              >
                {currentStep === 'complete' ? (
                  <Ionicons name="checkmark" size={20} color="white" />
        ...
        <View className="px-6 py-4 bg-purple-50 dark:bg-purple-900/20 border-t border-purple-200 dark:border-purple-800">
          <View className="flex-row items-start">
            <Ionicons name="information-circle" size={20} color="#9333EA" className="mt-0.5" />
            <View className="flex-1 ml-3">
              <Text variant="caption" className="text-purple-700 dark:text-purple-300">
                You&apos;ll be prompted to use Face ID or Touch ID to create your passkey. This is
                more secure than a PIN and makes transactions faster.
              </Text>
            </View>
          </View>
        </View>
