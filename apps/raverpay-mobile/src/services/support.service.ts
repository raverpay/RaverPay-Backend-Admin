...
export const supportService = {
  ...
  async markMessagesAsRead(conversationId: string): Promise<{ success: boolean }> {
  ...
  async getTickets(params?: {
    ...
    const response = await apiClient.get<TicketsResponse>(API_ENDPOINTS.SUPPORT.TICKETS, {
      params,
    });
    return response.data;
  },
  ...
  async getTicketById(ticketId: string): Promise<Ticket> {
    const response = await apiClient.get<Ticket>(API_ENDPOINTS.SUPPORT.TICKET_DETAIL(ticketId));
    return response.data;
  },
  ...
  async getUnreadCount(): Promise<UnreadCountResponse> {
    const response = await apiClient.get<UnreadCountResponse>(API_ENDPOINTS.SUPPORT.UNREAD_COUNT);
    return response.data;
  },
};
...
export const helpService = {
  ...
  async getCollections(): Promise<HelpCollection[]> {
    const response = await apiClient.get<HelpCollection[]>(API_ENDPOINTS.HELP.COLLECTIONS);
    return response.data;
  },
  ...
  async getArticleById(articleId: string): Promise<HelpArticle> {
    const response = await apiClient.get<HelpArticle>(API_ENDPOINTS.HELP.ARTICLE_DETAIL(articleId));
    return response.data;
  },

  async getArticleBySlug(slug: string): Promise<HelpArticle> {
    const response = await apiClient.get<HelpArticle>(API_ENDPOINTS.HELP.ARTICLE_BY_SLUG(slug));
    return response.data;
  },

  async searchArticles(query: string, limit?: number): Promise<HelpArticle[]> {
    const response = await apiClient.get<HelpArticle[]>(API_ENDPOINTS.HELP.SEARCH, {
      params: { query, limit },
    });
    return response.data;
  },
  ...
  async getPopularArticles(limit?: number): Promise<HelpArticle[]> {
    const response = await apiClient.get<HelpArticle[]>(API_ENDPOINTS.HELP.POPULAR, {
      params: { limit },
    });
    return response.data;
  },

  async markArticleHelpful(articleId: string, helpful: boolean): Promise<{ success: boolean }> {
    const response = await apiClient.post<{ success: boolean }>(
