...
export const PINModal: React.FC<PINModalProps> = ({
  ...
  useEffect(() => {
    const checkBiometric = async () => {
      ...
      if (enabled === 'true' && available) {
        const savedPin = await secureStorage.getItem(SECURE_KEYS.BIOMETRIC_TRANSACTION_PIN);
  ...
  }, [visible, pinLength, isBiometricEnabled, isBiometricAvailable, hasSavedPin]);
  ...
  const handleBiometricAuth = React.useCallback(async () => {
    ...
    try {
      ...
      // Retrieve stored PIN
      const storedPin = await secureStorage.getItem(SECURE_KEYS.BIOMETRIC_TRANSACTION_PIN);
      ...
      if (isBiometricEnabled && isBiometricAvailable) {
        try {
          ...
          if (!storedPin) {
            ...
            await secureStorage.setItem(SECURE_KEYS.BIOMETRIC_TRANSACTION_PIN, pinValue);
  ...
  const showBiometricButton = isBiometricAvailable && isBiometricEnabled && !loading;
  const showBiometricOnly = showBiometricButton && hasSavedPin && !showManualEntry;
      ...
      >
        <BlurView intensity={20} className="flex-1 justify-center items-center px-5">
          ...
          <View className="bg-white dark:bg-gray-800 rounded-3xl p-6 w-full max-w-sm">
            ...
            >
              <Ionicons name="close" size={20} color={loading ? '#6B7280' : '#5b55f6'} />
            </TouchableOpacity>
            ...
            {showBiometricButton && (
              ...
              >
                {biometricAuthInProgress ? 'Authenticating...' : `Use ${biometricType}`}
              </Button>
            )}
            ...
            {showBiometricOnly && (
              <TouchableOpacity
                ...
                disabled={loading || biometricAuthInProgress}
              >
                <Text variant="caption" align="center" className="text-gray-600">
                  Use PIN instead
                </Text>
              </TouchableOpacity>
            )}
